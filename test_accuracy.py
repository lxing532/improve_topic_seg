...
def main(args):
    ...
    if args.wiki:
        #dataset_folders = [Path('/ubc/cs/research/nlp/Linzi/transformer_TS/data/discussions_no_indicator/test_no_dup')]
        dataset_folders = [Path('/ubc/cs/research/nlp/Linzi/seg/wiki_section/test_wiki')]
        if (args.wiki_folder):
            dataset_folders = []
    ...
    for dataset_path in dataset_folders:
        ...
        save_path = '/ubc/cs/research/nlp/Linzi/transformer_TS/'
        f1 = open("/ubc/cs/research/nlp/Linzi/transformer_TS/paths.txt","w+")
        f2 = open("/ubc/cs/research/nlp/Linzi/transformer_TS/predictions.txt","w+")
        f3 = open("/ubc/cs/research/nlp/Linzi/transformer_TS/references.txt","w+")
        ...
        with tqdm(desc='Testing', total=len(dl)) as pbar:
            ...
            for i, (data, targets, paths, target_idx) in enumerate(dl):
                if i == args.stop_after:
                    break

                pbar.update()
                output,_ = model(data, target_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bs', help='Batch size', type=int, default=5)
    ...
    parser.add_argument('--seg_threshold', help='Threshold for binary classificetion', type=float, default=0.2)
